...
def learn(xs, ts, w, b, w2, b2, w_out, b_out, lr, iteration):
    ...
    for _ in range(iteration):
        ...
        #print("En_b:", En_b)
        En_z2 = np.dot(En_out.reshape(-1, 1), w_out.reshape(1, -1)) #二乗誤差のz2に関する微分 = dys/dz2 * dE/dys
        w_out -= En_w * lr
        b_out -= En_b * lr
        ...
        En_b_2 = np.dot(np.ones(ts.shape[0]), En_y2)    #二乗誤差のbに関する微分 = dy2 / db2 * dE / dy
        #print("En_b_1:", En_b_1)
        b2 -= En_b_2 * lr
        #print("w, b", w, b)
        ...
        w -= En_w_1 * lr
        En_b_1 = np.dot(np.ones(ts.shape[0]), En_y1)    #二乗誤差のbに関する微分 = dy1 / db1 * dE / dy1
        b -= En_b_1 * lr
